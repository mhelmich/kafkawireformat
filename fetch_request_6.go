...
type FetchRequest6_Partitions struct {
    Partition int32
    FetchOffset int64
...
func (that *FetchRequest6_Partitions) Encode(enc *Encoder) error {
...
func (that *FetchRequest6_Partitions) Decode(dec *Decoder) error {
...
type FetchRequest6_Topics struct {
    Topic string
    Partitions []*FetchRequest6_Partitions
}

func (that *FetchRequest6_Topics) Encode(enc *Encoder) error {
...
func (that *FetchRequest6_Topics) Decode(dec *Decoder) error {
    that.Topic = dec.ReadString()
    {
        ...
        if int(arrayLength) == -1 {
            ...
            buf := make([]*FetchRequest6_Partitions, arrayLength)
            var i int32
            for i = 0; i < arrayLength; i++ {
                item := new(FetchRequest6_Partitions)
                item.Decode(dec)
                buf[i] = item
            }
            that.Partitions = buf
...
type FetchRequest6 struct {
    ReplicaId int32
    MaxWaitTime int32
    MinBytes int32
    MaxBytes int32
    IsolationLevel int8
    Topics []*FetchRequest6_Topics
}

func (that *FetchRequest6) Encode(enc *Encoder) error {
...
func (that *FetchRequest6) Decode(dec *Decoder) error {
    ...
    that.IsolationLevel = dec.ReadInt8()
    {
        ...
        if int(arrayLength) == -1 {
            ...
            buf := make([]*FetchRequest6_Topics, arrayLength)
            var i int32
            for i = 0; i < arrayLength; i++ {
                item := new(FetchRequest6_Topics)
                item.Decode(dec)
                buf[i] = item
            }
            that.Topics = buf
