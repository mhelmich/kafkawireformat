...
type JoinGroupResponse2_Members struct {
    MemberId string
    MemberMetadata []byte
}

func (that *JoinGroupResponse2_Members) Encode(enc *Encoder) error {
    enc.WriteString(that.MemberId)
    enc.WriteByteArray(that.MemberMetadata)
    return nil
}

func (that *JoinGroupResponse2_Members) Decode(dec *Decoder) error {
    that.MemberId = dec.ReadString()
    that.MemberMetadata = dec.ReadByteArray()
    return nil
}

type JoinGroupResponse2 struct {
    ...
    LeaderId string
    MemberId string
    Members []*JoinGroupResponse2_Members
}

func (that *JoinGroupResponse2) Encode(enc *Encoder) error {
...
func (that *JoinGroupResponse2) Decode(dec *Decoder) error {
    ...
    that.MemberId = dec.ReadString()
    {
        ...
        if int(arrayLength) == -1 {
            ...
            buf := make([]*JoinGroupResponse2_Members, arrayLength)
            var i int32
            for i = 0; i < arrayLength; i++ {
                item := new(JoinGroupResponse2_Members)
                item.Decode(dec)
                buf[i] = item
            }
            that.Members = buf
