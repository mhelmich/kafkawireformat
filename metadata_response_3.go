...
type MetadataResponse3_Brokers struct {
    NodeId int32
    Host string
    Port int32
    Rack string
...
func (that *MetadataResponse3_Brokers) Encode(enc *Encoder) error {
...
func (that *MetadataResponse3_Brokers) Decode(dec *Decoder) error {
...
type MetadataResponse3 struct {
    ThrottleTimeMs int32
    Brokers []*MetadataResponse3_Brokers
    ClusterId string
    ControllerId int32
    TopicMetadata []*MetadataResponse3_TopicMetadata
}

func (that *MetadataResponse3) Encode(enc *Encoder) error {
...
func (that *MetadataResponse3) Decode(dec *Decoder) error {
    that.ThrottleTimeMs = dec.ReadInt32()
    {
        ...
        if int(arrayLength) == -1 {
            ...
            buf := make([]*MetadataResponse3_Brokers, arrayLength)
            var i int32
            for i = 0; i < arrayLength; i++ {
                item := new(MetadataResponse3_Brokers)
                item.Decode(dec)
                buf[i] = item
            }
            that.Brokers = buf
    ...
    that.ControllerId = dec.ReadInt32()
    {
        ...
        if int(arrayLength) == -1 {
            ...
            buf := make([]*MetadataResponse3_TopicMetadata, arrayLength)
            var i int32
            for i = 0; i < arrayLength; i++ {
                item := new(MetadataResponse3_TopicMetadata)
                item.Decode(dec)
                buf[i] = item
            }
...
type MetadataResponse3_PartitionMetadata struct {
    ErrorCode int16
    Partition int32
    Leader int32
    Replicas []int32
...
func (that *MetadataResponse3_PartitionMetadata) Encode(enc *Encoder) error {
...
func (that *MetadataResponse3_PartitionMetadata) Decode(dec *Decoder) error {
...
type MetadataResponse3_TopicMetadata struct {
    ErrorCode int16
    Topic string
    IsInternal bool
    PartitionMetadata []*MetadataResponse3_PartitionMetadata
}

func (that *MetadataResponse3_TopicMetadata) Encode(enc *Encoder) error {
    enc.WriteInt16(that.ErrorCode)
...
func (that *MetadataResponse3_TopicMetadata) Decode(dec *Decoder) error {
    that.ErrorCode = dec.ReadInt16()
    ...
    that.IsInternal = dec.ReadBool()
    {
        ...
        if int(arrayLength) == -1 {
            ...
            buf := make([]*MetadataResponse3_PartitionMetadata, arrayLength)
            var i int32
            for i = 0; i < arrayLength; i++ {
                item := new(MetadataResponse3_PartitionMetadata)
                item.Decode(dec)
                buf[i] = item
            }
